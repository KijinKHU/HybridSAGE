...
def train(dataset, args):
    ...
    for epoch_id in range(args.num_epochs):
        ...
        with torch.no_grad():
            ...
            evaluated_score = evaluation.evaluate_nn(dataset, h_item, args.k, args.batch_size)

            print(f'Epoch:{epoch_id}\tevaluated_score:{evaluated_score:.5f}')
            recoder = open(f"hybridSAGE_{item_ntype}_results.txt", 'a')
            recoder.write(f'\nEpoch:{epoch_id}\tevaluated_score:{evaluated_score:.5f}')
            recoder.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', type=str, default='cuda:0')        # "cpu"도 가능
